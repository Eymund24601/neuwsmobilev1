...
final topStoriesProvider = AsyncNotifierProvider<TopStoriesNotifier, List<ArticleSummary>>(
...
class TopStoriesNotifier extends CachedAsyncNotifier<List<ArticleSummary>> {
  ...
  List<ArticleSummary> decodePayload(Object? payload) {
    ...
    return list.map((item) => ArticleSummary.fromJson(item as Map<String, dynamic>)).toList();
  }
...
final articleDetailBySlugProvider = AsyncNotifierProviderFamily<ArticleDetailBySlugNotifier, ArticleDetail?, String>(
  ArticleDetailBySlugNotifier.new,
);

class ArticleDetailBySlugNotifier extends FamilyAsyncNotifier<ArticleDetail?, String> {
  @override
  Future<ArticleDetail?> build(String slug) async {
    final cache = await ref.read(cacheServiceProvider.future);
    final key = 'articleDetail:$slug:v1';
    final cached = await cache.read<ArticleDetail?>(
      key,
      (payload) {
        if (payload == null) {
          return null;
    ...
    if (cached != null) {
      if (DateTime.now().difference(cached.updatedAt) >= const Duration(minutes: 5)) {
  ...
  Future<ArticleDetail?> _refresh(String slug, CacheService cache) async {
    final fresh = await ref.read(articleRepositoryProvider).getArticleDetailBySlug(slug);
...
final tracksProvider = AsyncNotifierProvider<TracksNotifier, List<TrackSummary>>(TracksNotifier.new);
...
class TracksNotifier extends CachedAsyncNotifier<List<TrackSummary>> {
  ...
  List<TrackSummary> decodePayload(Object? payload) {
    final list = payload as List<dynamic>;
    return list.map((item) => TrackSummary.fromJson(item as Map<String, dynamic>)).toList();
  }
...
final trackModulesProvider = AsyncNotifierProviderFamily<TrackModulesNotifier, List<LearningModuleSummary>, String>(
  TrackModulesNotifier.new,
);

class TrackModulesNotifier extends FamilyAsyncNotifier<List<LearningModuleSummary>, String> {
  @override
  Future<List<LearningModuleSummary>> build(String trackId) async {
    final cache = await ref.read(cacheServiceProvider.future);
    final key = 'trackModules:$trackId:v1';
    final cached = await cache.read<List<LearningModuleSummary>>(key, (payload) {
      final list = payload as List<dynamic>;
      return list
          .map((item) => LearningModuleSummary.fromJson(item as Map<String, dynamic>))
          .toList();
    });
    if (cached != null) {
      if (DateTime.now().difference(cached.updatedAt) >= const Duration(minutes: 5)) {
  ...
  Future<List<LearningModuleSummary>> _refresh(String trackId, CacheService cache) async {
    final fresh = await ref.read(learnRepositoryProvider).getTrackModules(trackId);
...
final lessonProvider = AsyncNotifierProviderFamily<LessonNotifier, LessonContent?, String>(LessonNotifier.new);
...
class LessonNotifier extends FamilyAsyncNotifier<LessonContent?, String> {
  ...
  Future<LessonContent?> build(String lessonId) async {
    ...
    final cached = await cache.read<LessonContent?>(
      key,
      (payload) {
        if (payload == null) {
          return null;
  ...
  Future<LessonContent?> _refresh(String lessonId, CacheService cache) async {
    final fresh = await ref.read(learnRepositoryProvider).getLessonById(lessonId);
...
final eventsProvider = AsyncNotifierProvider<EventsNotifier, List<EventSummary>>(EventsNotifier.new);
...
class EventsNotifier extends CachedAsyncNotifier<List<EventSummary>> {
  ...
  List<EventSummary> decodePayload(Object? payload) {
    final list = payload as List<dynamic>;
    return list.map((item) => EventSummary.fromJson(item as Map<String, dynamic>)).toList();
  }
...
final eventDetailProvider = AsyncNotifierProviderFamily<EventDetailNotifier, EventSummary?, String>(
...
class EventDetailNotifier extends FamilyAsyncNotifier<EventSummary?, String> {
  ...
  Future<EventSummary?> build(String eventId) async {
    ...
    final cached = await cache.read<EventSummary?>(
      key,
      (payload) {
        if (payload == null) {
          return null;
  ...
  Future<EventSummary?> _refresh(String eventId, CacheService cache) async {
    final fresh = await ref.read(eventsRepositoryProvider).getEventById(eventId);
...
final creatorStudioProvider = AsyncNotifierProvider<CreatorStudioNotifier, CreatorStudioSnapshot>(
...
final quizCategoriesProvider = AsyncNotifierProvider<QuizCategoriesNotifier, List<String>>(
...
final quizByIdProvider = AsyncNotifierProviderFamily<QuizByIdNotifier, QuizSummary?, String>(
...
class QuizByIdNotifier extends FamilyAsyncNotifier<QuizSummary?, String> {
  ...
  Future<QuizSummary?> build(String quizId) async {
    ...
    final cached = await cache.read<QuizSummary?>(
      key,
      (payload) {
        if (payload == null) {
          return null;
...
final topicFeedProvider = AsyncNotifierProviderFamily<TopicFeedNotifier, TopicFeed, String>(
...
class TopicFeedNotifier extends FamilyAsyncNotifier<TopicFeed, String> {
  ...
  Future<TopicFeed> _refresh(String topicOrCountryCode, CacheService cache) async {
    final fresh = await ref.read(articleRepositoryProvider).getTopicFeed(topicOrCountryCode);
...
final startupPrefetchProvider = FutureProvider<void>((ref) async {
  ...
  if (topStories.isNotEmpty) {
    unawaited(ref.read(articleDetailBySlugProvider(topStories.first.slug).future));
  }
  ...
  if (categories.isNotEmpty) {
    final quizzes = await ref.read(gamesRepositoryProvider).getQuizzesByCategory(categories.first);
